...
class Module_List extends oxAdminList
{
    ...
    public function render()
    {
        ...
        foreach ( glob( $sModulesDir."*" ) as $sModule ) {

            // skipping oxfuncions file
            if ( basename($sModule) == "functions.php") {
                continue;
            }

            $oModule = oxNew( "oxModule" );

            if ( $oModule->load( basename($sModule) ) ) {
                $aModulesList[$sModule] = $oModule;
            }
        }
        ...
        // assign our list
        $this->_aViewData['mylist'] = $aModulesList;
