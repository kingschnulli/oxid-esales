                        ...
                        [{oxhasrights ident="SHOWARTICLEPRICE"}]
                            ...
                            [{block name="widget_product_listitem_infogrid_price_value"}]
                                [{if $product->getFPrice()}]
                                    <span class="price"><span>[{ $product->getFPrice() }]</span>
                                    [{if $oView->isVatIncluded() }]
                                        [{ $currency->sign}] [{if !($product->hasMdVariants() || ($oViewConf->showSelectListsInList() && $product->getSelections(1)) || $product->getVariants())}]*[{/if}]</span>
                                    [{/if}]
                                [{/if}]
                            [{/block}]
