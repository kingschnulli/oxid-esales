 ...
 * @version   SVN: $Id: oxarticle.php 48786 2012-08-17 10:20:42Z tomas $
 */
...
class oxArticle extends oxI18n implements oxIArticle, oxIUrl
{
    ...
    public function getFTPrice()
    {
        if ( $oPrice = $this->getTPrice() ) {
            if ( $oPrice->getBruttoPrice() ) {
                return oxRegistry::getLang()->formatCurrency( oxRegistry::getUtils()->fRound($oPrice->getBruttoPrice()));
            }
        }
    }
    ...
    public function getFPrice()
    {
        if ( $oPrice = $this->getPrice() ) {
            return $this->getPriceFromPrefix().oxRegistry::getLang()->formatCurrency( $oPrice->getBruttoPrice() );
        }
    }
    ...
    public function getPriceFromPrefix()
    {
        ...
        if ( $this->_blIsRangePrice) {
            $sPricePrefix = oxRegistry::getLang()->translateString('priceFrom').' ';
        }
    ...
    protected function _assignPrices()
    {
        ...
        if ((double) $this->oxarticles__oxunitquantity->value && $this->oxarticles__oxunitname->value) {
            ...
            $oCur = $myConfig->getActShopCurrencyObject();
            $this->_fPricePerUnit = oxRegistry::getLang()->formatCurrency($dPrice / (double) $this->oxarticles__oxunitquantity->value, $oCur);
        }
