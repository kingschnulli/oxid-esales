 ...
 * @version   SVN: $Id: oxbasket.php 48928 2012-08-22 13:44:33Z tomas $
 */
...
class oxBasket extends oxSuperCfg
{
    ...
    {
        return oxRegistry::getLang()->formatCurrency( $this->getDiscountedNettoPrice(), $this->getBasketCurrency() );
    }
    ...
    public function getFProductsPrice()
    {
        if ( $this->_oProductsPriceList ) {
            return oxRegistry::getLang()->formatCurrency( $this->_oProductsPriceList->getBruttoSum(), $this->getBasketCurrency() );
        }
        return null;
    }
    ...
    public function getWrappCostVat()
    {
        ...
        if ( $dWrappVAT > 0 && $this->getConfig()->getConfigParam( 'blShowVATForWrapping' ) ) {
            return oxRegistry::getLang()->formatCurrency( $dWrappVAT, $this->getBasketCurrency() );
        }
        return false;
    ...
    public function getWrappCostNet()
    {
        ...
        if ( $dWrappNet > 0 && $this->getConfig()->getConfigParam( 'blShowVATForWrapping' ) ) {
            return  oxRegistry::getLang()->formatCurrency( $dWrappNet, $this->getBasketCurrency() );
        }
        return false;
    }
    ...
    public function getFWrappingCosts()
    {
        ...
        if ( $oWrappingCost && $oWrappingCost->getBruttoPrice() ) {
            return oxRegistry::getLang()->formatCurrency( $oWrappingCost->getBruttoPrice(), $this->getBasketCurrency() );
        }
        return false;
    }
    ...
    public function getFDeliveryCosts()
    {
        ...
        if ( $oDeliveryCost && ( $this->getBasketUser() || $this->getConfig()->getConfigParam( 'blCalculateDelCostIfNotLoggedIn' ) ) ) {
            return oxRegistry::getLang()->formatCurrency( $oDeliveryCost->getBruttoPrice(), $this->getBasketCurrency() );
        }
        return false;
    }
