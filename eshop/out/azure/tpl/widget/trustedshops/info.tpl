...
[{if $tsId }]
    <div id="tsSeal">
        <a id="ts.certificate" class="external" href="https://www.trustedshops.com/shop/certificate.php?shop_id=[{$tsId}]">
            <img src="[{$oViewConf->getImageUrl()}]trustedshops_m.gif" title="[{ oxmultilang ident="WIDGET_TRUSTEDSHOPS_ITEM_IMGTITLE" }]">
        </a>
    </div>
    <div id="tsText">
        <a id="ts.profile" class="external" title="[{ oxmultilang ident="WIDGET_TRUSTEDSHOPS_ITEM_ALTTEXT" }]" href="[{ oxmultilang ident="WIDGET_TRUSTEDSHOPS_ITEM_PROFILELINK" }][{$tsId}].html">
            [{$oxcmp_shop->oxshops__oxname->value}] [{ oxmultilang ident="WIDGET_TRUSTEDSHOPS_ITEM_SEALOFAPPROVAL" }]
        </a>
    </div>
[{else}]
    <a id="ts.membership" class="external" href="[{ oxmultilang ident="WIDGET_TRUSTEDSHOPS_ITEM_LINK" }]">
