 ...
 * @version   SVN: $Id: content.php 41650 2012-01-21 15:21:25Z arvydas.vapsva $
 */
...
class Content extends oxUBase
{
    ...
    */
    ...
    public function getContentPageTpl()
    {
        ...
        if ( $sContentId == $myConfig->getConfigParam( 'sRDFaBusinessEntityLoc' )) {
            $sTemplate = $this->_sBusinessTemplate;
        } elseif ( $sContentId == $myConfig->getConfigParam( 'sRDFaDeliveryChargeSpecLoc' )) {
            $sTemplate = $this->_sDeliveryTemplate;
        } elseif ( $sContentId == $myConfig->getConfigParam( 'sRDFaPaymentChargeSpecLoc' )) {
            $sTemplate = $this->_sPaymentTemplate;
        }
        return $sTemplate;
    }
    ...
    */
    ...
    public function getRdfaPriceValidity()
    {
        ...
        $iFrom = oxUtilsDate::getInstance()->getTime() - (2*60*60);
        $iThrough = $iFrom + ($iDays * 24 * 60 * 60);
        $oPriceValidity = array();
        $oPriceValidity['validfrom'] = date('Y-m-d\TH:i:s', $iFrom);
        $oPriceValidity['validthrough'] = date('Y-m-d\TH:i:s', $iThrough);
