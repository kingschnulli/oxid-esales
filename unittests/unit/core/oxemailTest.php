 ...
 * @version   SVN: $Id: oxemailTest.php 33327 2011-02-17 12:58:04Z sarunas $
 */
...
//        modConfig::getInstance()->sTheme = false;
  ...
  protected function tearDown()
  {
    ...
    protected function checkMailBody( $sFuncName, $sBody, $blWriteToTestFile = false )
    {
        ...
        $sPath = getTestsBasePath().'/unit/email_templates/'.$sFuncName.'_'.$sVersion.'.html';
        if ( !($sExpectedBody = file_get_contents($sPath)) ) {
            return false;
        }

        if ($blWriteToTestFile) {
            file_put_contents ( getTestsBasePath().'/unit/email_templates/'.$sFuncName.'_testing.html', $sBody );
        }
